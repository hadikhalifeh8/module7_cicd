...
def predict_route(
    ...
    db: Session= Depends(get_db)  # Dependency to get the database session, if needed
):
    """
    ...
    # Create a new prediction record in the database. Use the 'sentiment' field to match DB schema.
    db_prediction_item = PredictionsModel(text=request.text, sentiment=result['label'])
...
def get_predictions(db: Session = Depends(get_db)):
    ...
    return [Prediction(text=pred.text, sentiment=pred.sentiment) for pred in predictions]